...
@Component({
  selector: 'app-tela-deposito',
  standalone: true,
  imports: [NgIf, CardComponent],
  templateUrl: './tela-deposito.component.html',
  styleUrls: ['./tela-deposito.component.css']
})
export class TelaDepositoComponent implements OnInit {
  erroMensagem = '';
  ...
  depositar(): void {
    ...
    this.http.post<DepositoResponse>(this.contaService.api() + '/deposit', body).subscribe({
      next: (response) => {
        this.contaService.currentBalance.set(response.current_balance);
        this.router.navigate(['/home']);
      },
      error: (err: HttpErrorResponse) => {
        this.erroMensagem = err.error?.detail ?? 'Erro ao realizar depósito.';
      }
    });
  }
